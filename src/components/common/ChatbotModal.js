...
const MODAL_HEIGHT = SCREEN_HEIGHT * 0.65 // 65% of screen height
...
const ChatbotModal = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    // useNativeDriver not supported on web, use false for web platform
    const useNativeDriver = Platform.OS !== 'web'
    
    if (visible) {
      Animated.spring(slideAnim, {
        toValue: isMinimized ? SCREEN_HEIGHT - MINIMIZED_HEIGHT : SCREEN_HEIGHT - MODAL_HEIGHT,
        useNativeDriver,
        tension: 50,
        friction: 7,
      }).start()
          ...
          <Animated.View
            style={[
              styles.modalContainer,
              {
                ...
                maxHeight: isMinimized ? MINIMIZED_HEIGHT : MODAL_HEIGHT,
              },
            ]}
          >
          {/* Header */}
