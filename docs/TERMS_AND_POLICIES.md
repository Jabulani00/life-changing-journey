# Life Changing Journey — Terms of Use, Privacy Policy & Legal Policies

**Single combined document** — Terms of Service, Privacy Policy, membership terms, booking terms, disclaimers, POPIA, and app-store compliance in one file.

**Document version:** 1.0  
**Last updated:** 19 May 2026  
**Public URL (when hosted):** `https://www.lifechangingjourney.co.za/legal/terms-and-policies`  
**Applies to:**

| Platform | Product |
|----------|---------|
| **Mobile app** | *Life Changing Journey* (Expo / React Native) — iOS App Store, Google Play, Huawei AppGallery |
| **Membership web** | Next.js app at `https://www.lifechangingjourney.co.za` (and staging/local equivalents) |
| **Related websites** | `lifechangingjourney.co.za`, partner service sites linked from the app |

---

> **Important — not legal advice**  
> This document was prepared from the project codebase and product documentation to support app-store submission and user transparency. **Life Changing Journey Organisation** should have a qualified attorney in **South Africa** review and customise this text before publication—especially registration details, tax/VAT, NCR/FSCA wording, professional indemnity, and POPIA registration if required.
...
2. [Terms of Service (Terms of Use)](#2-terms-of-service-terms-of-use)
3. [Privacy Policy](#3-privacy-policy)
4. [Membership & payment terms](#4-membership--payment-terms)
5. [Booking & scheduling terms](#5-booking--scheduling-terms)
6. [Acceptable use policy](#6-acceptable-use-policy)
7. [Health, wellness & professional services disclaimer](#7-health-wellness--professional-services-disclaimer)
8. [Financial services disclaimer](#8-financial-services-disclaimer)
9. [Cookie & similar technologies policy (web)](#9-cookie--similar-technologies-policy-web)
10. [Children & minors](#10-children--minors)
11. [Protection of Personal Information Act (POPIA)](#11-protection-of-personal-information-act-popia)
12. [Account deletion & data requests](#12-account-deletion--data-requests)
13. [Intellectual property](#13-intellectual-property)
14. [App store & platform notices](#14-app-store--platform-notices)
15. [Contact & data controller](#15-contact--data-controller)
16. [Publication checklist for store deployment](#16-publication-checklist-for-store-deployment)
...
| **“Third-party providers”** | External platforms that process data on our behalf (e.g. Google Firebase, Stripe, Calendly). |
...
- Paid Membership is primarily completed on the **Membership Web** via **Stripe Checkout** (South African Rand — ZAR).
- The Mobile App may display your membership status and entitlements and link you to the web for purchase or upgrade.
- Membership fees, tiers, benefits, discounts, and session limits are described in [Section 4](#4-membership--payment-terms) and in-app/on the website at the time of purchase.
...
- See [Section 5](#5-booking--scheduling-terms) for cancellation, rescheduling, and no-show rules.
...
See [Section 6 — Acceptable Use Policy](#6-acceptable-use-policy).
...
- **By you:** You may stop using the Services and request Account deletion (see [Section 12](#12-account-deletion--data-requests)).
...
See also [Section 7](#7-health-wellness--professional-services-disclaimer) and [Section 8](#8-financial-services-disclaimer).
...
**Contact:** See [Section 15](#15-contact--data-controller).
...
| Use payment processors (Stripe) for membership purchases on the web | Store full card numbers on LCJ servers (handled by Stripe) |
...
| Source | Data |
|--------|------|
| **Stripe** | Payment status, checkout session ID, amount, currency — not full card data |
...
2. Authenticate users and enforce entitlements;
3. Process membership purchases and record transactions;
4. Schedule, confirm, and manage bookings;
5. Communicate with you (responses, reminders where enabled, service updates);
6. Operate admin tools for staff (booking queue, tasks, events);
7. Improve security, prevent fraud and abuse;
8. Comply with legal obligations;
9. Send marketing only where you have **opted in** (you may opt out at any time).
...
| Recipient | Reason |
|-----------|--------|
| **Service providers** | Hosting (Google Cloud / Firebase), payments (Stripe), scheduling (Calendly), email delivery, AI API (Google Gemini — if chatbot enabled), Expo push infrastructure |
...
Firebase, Stripe, Calendly, and other providers may process data in countries outside South Africa (including the United States and European Union). Where required, we rely on appropriate safeguards (contractual clauses, provider certifications, and your consent where applicable).
...
- **Delete** information (see [Section 12](#12-account-deletion--data-requests));
...
- **Primary checkout:** Membership Web (`/plans`) via **Stripe Checkout**.
- **Mobile App:** Displays status and may deep-link or open the web for payment. The app does not store card details.
- When Stripe is not configured in a development environment, a **mock payment** mode may be used — not for production consumer billing.

### 4.3 Activation & entitlements

- Membership becomes **active** after successful payment confirmation (Stripe webhook / server verification).
...
- If `status` is not active or `endAt` has passed, entitlements may be suspended on the client and server.
...
- **Calendly** embed in the Mobile App (when `EXPO_PUBLIC_CALENDLY_EMBED_URL` is configured);
- Legacy or supplementary in-app booking flows writing to Firestore `bookings`;
- **Calendly webhooks** (Cloud Functions) syncing invitee.created / invitee.canceled events.
...
2. Impersonate any person or misrepresent affiliation with LCJ or partners;
3. Harass, threaten, defame, or discriminate against others;
4. Upload malware, scrape, reverse engineer, or attempt unauthorised access to systems;
5. Use the Services for spam, pyramid schemes, or unauthorised commercial solicitation;
6. Collect other users’ data without consent;
7. Submit false booking or payment information;
8. Circumvent membership entitlements, session limits, or security controls;
9. Use the AI assistant to generate illegal, harmful, or explicit content;
10. Record or distribute confidential sessions without consent of all parties and applicable law.
...
1. **Not medical care:** The Services and content (including articles, tips, chatbot responses, podcasts, and motivational materials) are for **general information, education, and support**. They **do not** constitute medical, psychiatric, psychological diagnosis, or treatment.

2. **Emergency:** If you are in crisis or may harm yourself or others, contact **emergency services immediately** (South Africa: **112** / **10111**) or a local crisis line. The app is **not** an emergency service.

3. **Qualified professionals:** Psychology, hypnotherapy, spiritual, and integrated services may be delivered by **independent practitioners** or partner entities. LCJ facilitates access and information; clinical responsibility lies with the licensed or qualified provider conducting the session.

4. **No guarantee of outcomes:** Wellness journeys are individual. We do not guarantee specific health, mental health, or spiritual outcomes.

5. **Your responsibility:** Consult a qualified healthcare provider before making health decisions. Do not discontinue prescribed treatment based on app content.

6. **Children:** Services for children require parental/guardian involvement and appropriate professional consent.
...
1. **Separate regulated entities:** Financial guidance, loans, and credit services promoted through LCJ may be offered by **Tshabalala Omhle Financial Group** or other **NCR-registered** providers (e.g. registration **NCRCP20083** where stated in-app). LCJ is **not** a registered credit provider unless separately stated.

2. **Not financial advice:** General content in the app is **not** personal financial advice, investment advice, or a recommendation to enter any credit agreement.

3. **Responsible lending:** Credit products are subject to **affordability assessments**, **National Credit Act** requirements, and the provider’s terms. “Financial services are subject to responsible lending practices” (in-app disclaimer).

4. **Third-party websites:** Links to `tshabalalafinance.co.za` and similar sites are governed by those entities’ policies.

5. **No guarantee:** LCJ does not guarantee loan approval, interest rates, or financial results.
...
**Third-party cookies:** Stripe and Firebase may set cookies during checkout/auth.
...
- `user_memberships/{uid}` (subject to financial record retention);
- Associated bookings/contacts where retention is not legally required.
...
- **Privacy Policy URL** (required): use this combined document at one public HTTPS URL, e.g. `https://www.lifechangingjourney.co.za/legal/terms-and-policies`
- **Terms of Use (EULA):** Link to the same URL, or use standard Apple EULA and reference this document in the app description
- **Account deletion:** Required for apps with account creation — implement link per Section 12
- **App Privacy labels:** Declare contact info, identifiers, user content, purchase history, and other data per actual collection (Firebase, Stripe redirect, Calendly, notifications)
- **Permissions declared** (`app.json`): phone, camera, photo library, location when in use — align Privacy Nutrition Labels with actual use
...
- Ensure APK signing and package name `com.lifechangingjourney.app` consistency
...
- [ ] Update `RegisterScreen` / login to open those URLs (replace plain text with links)
- [ ] Replace outdated `privacy.html` in repo root OR redirect it to the new policy
- [ ] App Store Connect: Privacy Policy URL, support URL, demo account for reviewers
